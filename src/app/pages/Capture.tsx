...
function saveToRoll(photo: string, note: string) {
  const existing = JSON.parse(localStorage.getItem("tintura_roll") ?? "[]");
  const entry = {
    id: `roll_${Date.now()}`,
    photo,
    note,
  ...
  localStorage.setItem("tintura_roll", JSON.stringify([entry, ...existing]));
}
...
export default function Capture() {
          ...
          {step === "camera" && (
            <>
              ...
              <button
                ...
                onMouseEnter={(e) =>
                  ((e.currentTarget as HTMLElement).style.transform = "translateX(-50%) scale(1.06)")
                }
                onMouseLeave={(e) =>
  ...
  return (
    ...
    >
      {/* Portrait photo — centered, not full-width */}
      {photo && (
        <div
          style={{
            maxWidth: 320,
      ...
      >
        ...
        <textarea
          ...
          onFocus={(e) => {
            if (!note) e.target.style.fontStyle = "normal";
          }}
          onBlur={(e) => {
            if (!note) e.target.style.fontStyle = "italic";
          }}
        />
