...
function extractKMeans(canvas: HTMLCanvasElement, k: number): string[] {
  ...
  for (let iter = 0; iter < 20; iter++) {
    ...
    for (const p of pixels) {
      ...
      sums[minI][0] += p[0]; sums[minI][1] += p[1]; sums[minI][2] += p[2]; sums[minI][3]++;
    }
    for (let j = 0; j < k; j++) {
      if (sums[j][3] > 0)
        centers[j] = [
...
function Swatch({ color, size, onRemove, showHex }: {
  color: string; size: number; onRemove?: () => void; showHex?: boolean;
}) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
      <div
        ...
        style={{ width: size, height: size, borderRadius: 4, background: color, border: "1px solid rgba(0,0,0,0.08)", position: "relative", flexShrink: 0 }}
      >
        {onRemove && hovered && (
          <button
            onClick={onRemove}
            style={{
              ...
              borderRadius: "50%", background: "#1A1814", border: "none", cursor: "pointer",
      ...
      {showHex && (
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--color-text-secondary)", letterSpacing: "0.02em" }}>
          {color}
        </span>
      )}
    </div>
...
function CanvasPicker({ photoUrl, swatches, onAddSwatch, onShake, canvasRef, developing }: {
  ...
  const drawLoupe = useCallback((canvasX: number, canvasY: number) => {
    ...
    lCtx.drawImage(canvas, Math.max(0, canvasX - src / 2), Math.max(0, canvasY - src / 2), src, src, 0, 0, loupe, loupe);
  ...
  function handleClick(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    onAddSwatch(toHex(pixel[0], pixel[1], pixel[2]));
  }

  const LOUPE_SIZE = 80;
  const LOUPE_OFFSET = 16;
  const nearTop = loupePos.y < LOUPE_SIZE + LOUPE_OFFSET + 8;
  const loupeTop = nearTop ? loupePos.y + LOUPE_OFFSET : loupePos.y - LOUPE_SIZE - LOUPE_OFFSET;

  return (
    <div
      ref={containerRef}
      style={{ position: "relative", width: "100%", paddingTop: "133.33%", borderRadius: 4, overflow: "hidden", border: "1px solid #E8E4DC", background: "#E0DEDA", boxShadow: "0 1px 4px rgba(0,0,0,0.07), 0 4px 20px rgba(0,0,0,0.05)" }}
    >
      <canvas
        ref={canvasRef}
      ...
      {loupeVisible && imageReady && (
        <div
          style={{
            position: "absolute", left: loupePos.x - LOUPE_SIZE / 2, top: loupeTop,
            width: LOUPE_SIZE, height: LOUPE_SIZE, borderRadius: "50%", overflow: "hidden",
            border: "2px solid rgba(255,255,255,0.85)", boxShadow: "0 2px 12px rgba(0,0,0,0.3)",
            pointerEvents: "none", zIndex: 10,
          }}
        >
          <canvas ref={loupeCanvasRef} width={LOUPE_SIZE} height={LOUPE_SIZE} style={{ display: "block" }} />
        </div>
      )}
    </div>
  );
...
export default function Curation() {
  ...
  const diaryEntry = getDiary().find((e) => e.id === id);
  const rollEntry = getRoll().find((e) => e.id === id);
  const isEditMode = !!diaryEntry;

  const photoUrl = diaryEntry?.photo ?? rollEntry?.photo ?? "";
  const initialNote = diaryEntry?.name ?? rollEntry?.note ?? "";
  const initialSwatches: string[] = isEditMode ? (diaryEntry?.swatches ?? []) : [];
  ...
  function handleDevelop() {
    ...
    if (isEditMode && diaryEntry) {
      updateDiaryEntry(diaryEntry.id, { swatches, name: name.trim() || initialNote });
      navigate(returnTo);
      return;
    }

    // Develop: animate unblur, save to diary, remove from roll
    setDeveloping(true);
    setTimeout(() => {
      const entry: DiaryEntry = {
        id: `dev_${crypto.randomUUID()}`,
        photo: photoUrl || null,
        name: name.trim() || initialNote,
        swatches,
      ...
      addToDiary(entry);
      if (id) removeFromRoll(id);
      navigate("/");
    }, 700);
  }

  // No photo and no entry found — probably a bad URL
  if (!photoUrl && !isEditMode && !rollEntry) {
    return (
      <>
        <BackNav label="back" to={returnTo} />
        <main style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 48px" }}>
          <p style={{ fontFamily: "var(--font-display)", fontWeight: 300, fontStyle: "italic", fontSize: 20, color: "var(--color-text-secondary)" }}>
            Entry not found.
          </p>
        </main>
      </>
    ...
    <>
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 48px 120px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "55fr 45fr", gap: 64, alignItems: "start" }}>
          ...
          {/* ── Left: canvas + swatch strip ── */}
          <div>
            <span style={{ display: "block", fontFamily: "var(--font-ui)", fontWeight: 400, fontSize: 12, color: "#8C8880", marginBottom: 8 }}>
            ...
            />
            {/* Small swatch strip below canvas */}
            <div style={{ marginTop: 16, minHeight: 52, display: "flex", alignItems: "center", gap: 8 }}>
              <div className={shake ? "shake" : ""} style={{ display: "flex", gap: 8 }}>
                {swatches.map((color, i) => (
                  <Swatch key={i} color={color} size={36} onRemove={() => handleRemoveSwatch(i)} />
                ))}
                {Array.from({ length: 5 - swatches.length }).map((_, i) => (
                  <div key={`empty-${i}`} style={{ width: 36, height: 36, borderRadius: 4, border: "1px dashed #D4CEBF", background: "#F0EDE8" }} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div style={{ paddingTop: 4, paddingLeft: 40, paddingRight: 24 }}>
            ...
            {/* 01 PICK YOUR COLORS */}
            <p style={{ fontFamily: "var(--font-ui)", fontWeight: 500, fontSize: 11, color: "var(--color-text-secondary)", letterSpacing: "0.08em", textTransform: "uppercase", margin: "0 0 12px" }}>
              01 Pick your colors
            </p>
            <p style={{ fontFamily: "var(--font-ui)", fontWeight: 400, fontSize: 11, color: "#8C8880", margin: "0 0 12px" }}>
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 16 }}>
              {Array.from({ length: 5 }).map((_, i) => {
                ...
                if (color) {
                  return <Swatch key={i} color={color} size={48} onRemove={() => handleRemoveSwatch(i)} showHex />;
                }
                return (
                  <div key={`slot-${i}`} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                    <div style={{ width: 48, height: 48, borderRadius: 4, border: "1px dashed #D4CEBF", background: "#F0EDE8", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            <button
              ...
              onMouseEnter={(e) => { if (swatches.length < 5) (e.currentTarget as HTMLElement).style.color = "#1A1814"; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = swatches.length >= 5 ? "#C8C4BC" : "#8C8880"; }}
            >
            ...
            <div style={{ marginTop: 32, marginBottom: 32 }}>
              ...
              <input
                ...
                style={{
                  ...
                  fontSize: 22, lineHeight: 1.38, color: "#1A1814",
                  background: "transparent", border: "none", borderBottom: "1px solid #C4A882",
                  outline: "none", padding: "0 0 8px", letterSpacing: "-0.01em",
                  boxSizing: "border-box", caretColor: "#C4A882",
                }}
              />
              <p style={{ fontFamily: "var(--font-ui)", fontWeight: 400, fontStyle: "italic", fontSize: 11, color: "#8C8880", margin: "8px 0 0" }}>
            ...
            <div style={{ marginTop: 32 }}>
              ...
              <button
                ...
                style={{
                  width: "100%", fontFamily: "var(--font-ui)", fontWeight: 500, fontSize: 14,
                  ...
                  letterSpacing: "0.02em", transition: "background 200ms ease, color 200ms ease",
                }}
              >
                {developing ? "Developing…" : isEditMode ? "Save changes" : "Develop this memory"}
              </button>
              {swatches.length === 0 && (
                <p style={{ fontFamily: "var(--font-ui)", fontWeight: 400, fontSize: 11, color: "#C4A882", marginTop: 6, textAlign: "center" }}>
